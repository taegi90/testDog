...
<head>
    ...
    <script src="/dog/js/jquery.imagesloaded.min.js"></script>
    <script src="/dog/js/jquery.masonry.min.js"></script>
    <style>
        * {
            margin:0;padding:0;
            ...
            $('.paper').click(function() {
            	...
            	*/
            	...
            	$.ajax({
            		...
            		dataType : 'JSON',
            		success : function(data) {
						$("#dog_kind").text(data.kind);
						$("#dog_price").text(data.price);
						$("#dog_image").attr("src", data.image);
            		}
            	});
        ...
        function ajaxView(id) {
        	$.ajax({
        		...
        		success : function(data) {
					...
					$("#dog_kind").text(data.kind);
					$("#dog_price").text(data.price);
					$("#dog_height").text(data.height);
					$("#dog_weight").text(data.weight);
					$("#dog_country").text(data.country);
					$("#dog_content").text(data.content);
					$("#dog_image").attr("src", data.image);
        		},
        		error : function(data) {
        			console.log(data);
...
<body>
    ...
    <nav id="main-navigation">
        <ul><li><a href="/dog/dog/registForm.do">강아지 등록하기</a>
    ...
    <c:forEach var="dog" items="${dogList}" varStatus="status">
        <div class="paper" data-id="${dog.id }" onclick="ajaxView('${dog.id}');">
            <div class="paper-holder">
                <a><img width="190" src="/dog/images/${dog.image }"></a>
            </div>
            <p class="paper-description">${dog.kind }</p>
            <div class="paper-content">
                <p class="paper-text">가격 : ${dog.price }원</p>
            </div>
        </div>
     </c:forEach>
    </section>
       ...
       <div class="paper" data-id="${dog.id }" onclick="ajaxView('${dog.id}');">
            <div class="user-information">
                <div class="user-information-text">
                    <h3 id="dog_kind">${dog.kind }</h3>
                    <p id="dog_price">${dog.price }</p>
                    <p id="dog_height">${dog.height }</p>
                    <p id="dog_weight">${dog.weight }</p>
                    <p id="dog_country">${dog.country }</p>
                    <p id="dog_content">${dog.content }</p>
                </div>
            </div>
            <hr class="lightbox-spllitter">
            <img id="dog_image" src="/dog/images/${dog.image }">
